...
static int handle_report_view(struct obd_agree_obj* const _agree_ofp, const struct general_pack_shanghai *const _pack, struct msg_print_obj* const _print)
{
...
static void upload_query(char* const vin, struct obd_agree_obj* const _agree_ofp, struct upload* const decode, const char* suffix, struct msg_print_obj* const _print)
{
    int len;
    char filename[128];
...
static void upload_download(struct obd_agree_obj* const _agree_ofp, struct upload* const decode, struct msg_print_obj* const _print)
{
    int len;
    char filename[128];
...
static int handle_request_userdef(struct obd_agree_obj* const _agree_ofp, const struct general_pack_shanghai* const _pack, struct msg_print_obj* const _print)
{
...
static void upload_query_yunjing(const char* const vin, struct obd_agree_obj* const _agree_ofp, struct yunjing_userdef* const _udef, const char _sn[], const uint32_t _dev_checksum, const char* suffix, struct msg_print_obj* const _print)
{
    int len;
    char filename[128];
    uint8_t buffer[sizeof(struct upload)];
...
static void upload_query_block_yunjing(const char* const vin, struct obd_agree_obj* const _agree_ofp, struct yunjing_userdef* const _udef, const char _sn[], const uint32_t _dev_checksum, const char* suffix, struct msg_print_obj* const _print)
{
    int len;
    char filename[128];
    uint8_t buffer[sizeof(struct upload)];
...
static void upload_download_yunjing(struct obd_agree_obj* const _agree_ofp, struct yunjing_userdef* const _udef, const char _filename[], const uint32_t _seek, const uint32_t _total, const uint16_t block, struct msg_print_obj* const _print)
{
    int len;
    char filename[128];
    //const char *suffix=NULL;
...
static int handle_userdef_yunjing_view(struct obd_agree_obj* const _agree_ofp, const struct general_pack_shanghai* const _pack, struct msg_print_obj* const _print)
{
    const struct shanghai_userdef *const msg = (const struct shanghai_userdef *const)(_pack->data);
    ...
    else
    {
        switch (_userdef.type_msg)
        {
            ...
            case USERDEF_YJ_QUERY_CFG:   // 查询配置文件更新
                ...
                upload_query_yunjing((char*)_pack->VIN, _agree_ofp, &_userdef, _qure->sn, _qure->checksum, ".cfg", _print);
                break;
            ...
            case USERDEF_YJ_QUERY_FW:    // 查询固件更新
                ...
                upload_query_yunjing((char*)_pack->VIN, _agree_ofp, &_userdef, _qure->sn, _qure->checksum, ".bin", _print);
                break;
            ...
            case USERDEF_YJ_QUERY_FWB:    // 查询固件更新,获取固件块信息
                ...
                upload_query_block_yunjing((char*)_pack->VIN, _agree_ofp, &_userdef, _qure->sn, _qure->checksum, ".bin", _print);
                break;
            ...
            case USERDEF_YJ_DOWNLOAD:     // 下载
                _print->fops->print(_print, "下载\n");
                {
                    ...
                    upload_download_yunjing(_agree_ofp, &_userdef, _download->key, _download->_seek, _download->_total, _download->block, _print);
                }
                break;
...
int obd_protocol_view_shanghai(struct obd_agree_obj* const _this, struct msg_print_obj* const _print)
{
    const char* ssl_type[] = {
        "0x00: NULL",
        "0x01：数据不加密",
    ...
    const struct general_pack_shanghai* const pack = &_this->_gen_pack;
    _this->_print = 1;
    ...
    {
        memset(_this->VIN, 0, sizeof (_this->VIN));
        memcpy(_this->VIN, pack->VIN, sizeof (pack->VIN));
    }
    _this->_relay = 1;
    //printf("@%s-%d \n", __func__, __LINE__);
    ...
    switch(pack->cmd)
    {
        ...
        case CMD_REPORT_REAL:  // 实时信息上报
            ...
            handle_report_view(_this, pack, _print);
            break;
        ...
        case CMD_USERDEF:      // 用户自定义
            ...
            handle_request_userdef(_this, pack, _print);
            _this->_relay = 0;
            break;
        default:
...
int obd_protocol_view_yunjing(struct obd_agree_obj* const _this, struct msg_print_obj* const _print)
{
    const char* ssl_type[] = {
        "0x00: NULL",
        "0x01：数据不加密",
    ...
    const struct general_pack_shanghai* const pack = &_this->_gen_pack_YJ;
    _this->_print = 1;
    _print->fops->print(_print, "起始符:[%c%c] ", pack->start[0], pack->start[1]);
    _print->fops->print(_print, "命令:[%02X] ", pack->cmd);
    _print->fops->print(_print, "VIN:[%-17s] ", pack->VIN);
    ...
    switch(pack->cmd)
    {
        ...
        case CMD_USERDEF_YJ:      // 用户自定义
            _print->fops->print(_print, "自定义-"); // fflush(stdout);
            handle_userdef_yunjing_view(_this, pack, _print);
            _this->_relay = 0;
            break;
...
int obd_protocol_view(struct obd_agree_obj* const _this, struct msg_print_obj* const _print)
{
    const struct general_pack_view* const pack = &_this->_gen_pack_view;
    _this->_print = 1;
    ...
    _print->fops->print(_print, "count:%3d ", pack->count);
    _print->fops->print(_print, "版本号:[%02X] ", pack->soft_version);
    _print->fops->print(_print, "数据长度:[%-4d] ", pack->data_len);
    _print->fops->print(_print, "BCC:[%02X] ", pack->BCC);
    if(17==strlen((char*)pack->VIN))
    ...
    {
        memset(_this->sn, 0, sizeof (_this->sn));
        memcpy(_this->sn, pack->sn, sizeof (pack->sn));
    }
    _this->_relay = 1;
    _print->fops->print(_print, "数据:");
    switch(pack->cmd)

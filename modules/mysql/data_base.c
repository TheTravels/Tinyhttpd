...
#if 0
    if((strlen(_db->sql_query)>10) && (sizeof(struct db_sql_pool)>strlen(_db->sql_query)))
    {
        memcpy(_db->sql_pool[_db->sql_pool_size].sql, _db->sql_query, strlen(_db->sql_query));
        _db->sql_pool_size++;
        if(_db->sql_pool_size>=64)
        {
            _db->fops->insert_sql(_db);
            _db->sql_pool_size=0;
        }
    }
#endif
...
static int data_base_fops_insert_sql(struct data_base_obj *const _db)
{
    ...
    if(strlen(_db->sql_query)<10) return 0;
...
static int data_base_fops_insert_sql(struct data_base_obj *const _db)
{
    ...
    {
        if(strlen(_db->sql_pool[i].sql)<10) continue;

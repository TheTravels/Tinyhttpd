...
#else
static inline int __list_add_valid(struct list_head *_new,
                struct list_head *prev,
                struct list_head *next)
{
    (void)_new;
			      ...
			      struct list_head *next)
{
    ...
    //printf("head_free: 0x%08X 0x%08X \n", next, next->prev); fflush(stdout);
    //printf("head_free: 0x%08X 0x%08X \n", prev, prev->next); fflush(stdout);
    next->prev = _new;
    _new->next = next;
    _new->prev = prev;
...
{
    //printf("list_add: 0x%08X 0x%08X 0x%08X \n", _new, head, head->next); fflush(stdout);
